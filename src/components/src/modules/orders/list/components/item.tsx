        ...
        primaryText={
          <div className="row middle-xs">
            ...
            <div className="col-xs-2">
              <Link to={`/order/${order.id}`} className="number">
                {order.number || 0}
              </Link>
              <br />
              <small className="small">{dateCreatedFromNow}</small>
            </div>
            <div className="col-xs-4">
              <div className="shipping">{shippingTo}</div>
              <small className="small">{order.shipping_method}</small>
            </div>
            <div className={`col-xs-2 $"price}`}>
              {grandTotalFormatted}
              <br />
              <small className="small">{order.payment_method}</small>
            </div>
            <div className={`col-xs-2 $"status"`}>{order.status}</div>
          </div>
        }
      />
      <Divider />
