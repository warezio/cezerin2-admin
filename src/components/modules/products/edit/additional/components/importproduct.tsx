...
const ProductImport = props => {
  ...
  function removeCategories() {
    let catArray = []
    api.productCategories.list().then(({ json }) => {
      catArray = json
      json.forEach(function (element) {
        api.productCategories.delete(element.id).then(() => {
    ...
    updateProductArray.some((pArrayItem, i) => {
      ...
      api.products
        ...
        .then(({ status, json }) => {
          ...
          that.setDeleteCounter(0)
          that.setUploadedProducts(i + 1)
          ...
          json.forEach(jFile => {
            if (imageFile === jFile.file) {
              ...
              xhr.onload = function (e) {
                ...
                let files = new File([blob], imageFile, {
                  type: "image/jpeg",
                })
  ...
  return (
    <>
      <div style={{ width: "100%" }}>
        <div className="spread-sheet-container" style={state.productsImport}>
